...
getUptime() 
{
  ...
  local last_boot_time=$(sysctl kern.boottime | awk '{print $5}' | sed 's/,//g')
  local sys_uptime=$((current_time - last_boot_time))
  local hours=$((sys_uptime / (60*60)))
  local minutes=$(((sys_uptime / 60) % 60 ))
